...
Write-capable tools are disabled by default. To expose tools that create, update, publish, or discard changes, set `ENABLE_WRITE=true` before starting the MCP server. Delete operations stay hidden unless `ENABLE_DESTROY=true` is also set.
